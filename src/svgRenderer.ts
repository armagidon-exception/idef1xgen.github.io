...
export function createSVGEntity(entity: Entity, options: SVGRenderOptions = {}): SVGGElement {
    ...
    entity.primaryKey.forEach(pk => {
        const pkGroup = createAttributeSVG(pk, true, opts.fontFamily!, opts.fontSize!, measurements.lineHeight, measurements.width);
        pkGroup.setAttribute("transform", `translate(${opts.padding}, ${currentY})`);
    ...
    entity.attributes.forEach(attr => {
        const attrGroup = createAttributeSVG(attr, false, opts.fontFamily!, opts.fontSize!, measurements.lineHeight, measurements.width);
        attrGroup.setAttribute("transform", `translate(${opts.padding}, ${currentY})`);
...
function createAttributeSVG(attr: Attribute, isPK: boolean, fontFamily: string, fontSize: number, lineHeight: number, maxWidth: number): SVGGElement {
    const group = document.createElementNS(SVG_NS, "g");
    ...
    const nameWidth = getTextWidth(attr.name + (attr.optional ? " (O)" : ""), fontFamily, fontSize);
...
function measureEntity(entity: Entity, opts: SVGRenderOptions): EntityMeasurements {
    ...
    allAttrs.forEach(attr => {
        ...
        const totalWidth = getTextWidth(name, opts.fontFamily!, opts.fontSize!) + 
    ...
    const totalHeight = padding + // top padding
                       (totalLines * lineHeight) + 
                       (entity.primaryKey.length > 0 || entity.attributes.length > 0 ? padding : 0) + // separator area
...
function getTextWidth(text: string, fontFamily: string, fontSize: number): number {
    ...
    const adjustedLength = text.length * 0.9 + text.replace(/[^il1]/g, '').length * 0.3;
    return adjustedLength * avgCharWidth;
}

export function renderEntitiesToSVG(entities: Entity[], container: SVGSVGElement, options?: SVGRenderOptions): void {
    container.innerHTML = "";
